...
fun artifactFileName(os: String, arch: String, version: String): String {
    val wasmEdgeSuffix = when {
        os.lowercase().contains("linux") -> when {
            arch == "x86_64" -> "manylinux_2_28_x86_64.tar.gz"
            arch == "arm" || arch.startsWith("aarch") -> "manylinux_2_28_aarch64.tar.gz"
            else -> error("unsupported os type ${os} and arch $arch")
        }
        os.lowercase().contains("mac") -> when {
            arch == "x86_64" -> "darwin_x86_64.tar.gz"
            arch == "arm" || arch.startsWith("aarch") -> "darwin_arm64.tar.gz"
            else -> error("unsupported os type ${os} and arch $arch")
        }
