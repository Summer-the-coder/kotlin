...
class KotlinWasmCustomRuntimeGradlePluginIT : KGPBaseTest() {
    ...
    @DisplayName("Check wasi target with deno")
    @GradleTest
    @GradleTestVersions(
        minVersion = TestVersions.Gradle.G_8_0
    )
    @TestMetadata("new-mpp-wasm-wasi-custom-runtime")
    fun wasiDenoTarget(gradleVersion: GradleVersion) {
        testWithCustomRuntime("deno", gradleVersion)
    }

    @DisplayName("Check wasi target with wasm-edge")
    @GradleTest
    @GradleTestVersions(
        minVersion = TestVersions.Gradle.G_8_0
    )
    ...
    fun wasiWasmEdgeTarget(gradleVersion: GradleVersion) {
        testWithCustomRuntime("wasmEdge", gradleVersion)
    }
